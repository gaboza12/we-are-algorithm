...
void Setting_Block(int Shape, int x, int y)
{
	...
	{
		int B_Idx = y + 1;
		while (B_Idx < 10 && Area[B_Idx][x][0] == 0)
			B_Idx++;
		Area[B_Idx - 1][x][0] = Figure_Num;

		int G_Idx = x + 1;
		while (G_Idx < 10 && Area[G_Idx][y][1] == 0)
			G_Idx++;
	...
	else if (Shape == 2)
	{
		...
		int G_Idx = x + 1;
		while (G_Idx < 10 && (Area[G_Idx][y][1] == 0 && Area[G_Idx][y + 1][1] == 0))
			G_Idx++;
		Area[G_Idx - 1][y][1] = Figure_Num;
	...
	{
		int B_Idx = y + 1;
		while (B_Idx < 10 && (Area[B_Idx][x][0] == 0 && Area[B_Idx][x + 1][0] == 0))
			B_Idx++;
		Area[B_Idx - 1][x][0] = Figure_Num;
			...
			{ 
				...
				{ 
					...
					{ 
						...
						if (Area[Temp_x][y][Color] != 0 || Temp_x >= 10)
							break;
					...
					{ 
						...
						if (Area[Temp_xx][yy][Color] != 0 || Temp_xx >= 10)
							break;
					...
					if (xx + Move_Cnt == x)
						...
						Area[x][y][Color] = Area[xx][yy][Color] = 0;
